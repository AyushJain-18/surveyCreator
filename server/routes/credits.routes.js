...
Route.post('/api/addCredit', authMiddelware, async (req, res) => {
  ...
  try {
    if (!req.user.stripe_customer_id) {
      ...
      userWithCustumerId = await User.findByIdAndUpdate(
        req.user._id,
    ...
    const charge = await Stripe.charges.create({
      ...
      customer: req.user.stripe_customer_id
        ...
        : userWithCustumerId.stripe_customer_id,
      receipt_email: req.body.token.email,
    });
    userWithNewCredits = await User.findByIdAndUpdate(
      req.user._id,
      { credit: req.user.credit + 5 },
      { new: true }
    );
  } catch (error) {
    console.log('//Error// \n', error);
