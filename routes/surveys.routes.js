    ...
    try{    
        let {surveyGeneratorId, reciverMailId, response} = req.body;
          ...
          survey.recipents.map((recipent, index) =>{
              console.log('1.inside map', recipent.email, reciverMailId, recipent.response);
            if(recipent.email === reciverMailId && !recipent.response ){
                console.log('2.inside map', recipent.email, reciverMailId, recipent.response);
            ...
            if(recipeintIndex !== null){
                ...
                response === 'Yes' ?  survey.TotalYes = survey.TotalYes+1 :  survey.TotalNo = survey.TotalNo+1;
        ...
        console.log('//Error occured// while updating survey response Surveys', error.message);
        return  res.status(500).send({survey: 'Error, while submitting your response'});
    }
